...
//                s.src = "./CircleCenter_1.png?version=1",
                if(poid>0) s.src = sysGame[2].bg;
                else s.src = sysGame[0].bg;
                lens = i.levelArray[1],
            ...
            gameStart: function() {
            	var that = this;
            	//体验模式
                if(poid>0) this.level = 3;
                else this.level = arguments[0] || 1;
                this.isPause = !1,
                this.isOver = !1,
                this.isCanTap = !0,
            ...
            gameContinue: function(t) {
                ...
                this.isOver = !1),
                this.update.call(this)
                setTimeout(function(){document.getElementById("levelSwitchBox").click();}, 100);
            },
            gameOver: function() {
            	var that = this;
            	...
            	$.ajax({
            		type:'post',
            		url:initUrl,
            		data:{op: 'fail', logId: logId, mid: nowMid, oid: nowOid, level: tmpLevel},
                ...
                if(!this.isPause&&!this.isOver){
                    ...
                    if(!this.isCanTap){
                        ...
                        if(tempBC.y <=  d.y + m + 20){
                        	...
                        	if(sparams==3 && !checkCollisionResult.isCollision && (tempBC.y =  d.y + o+10)){var fdirtem = ['left','right','bottom'];sparams=1;checkCollisionResult = {isCollision:true,failDirection:fdirtem[Math.floor(Math.random()*fdirtem.length)]}}
...
//            	this.bulletCircles[n] && (this.bulletCircles[n].imgObj.src = "./Sword_small_gray.png?version=1",
                ...
                n == lens) {
                    ...
                    $("#insert_audio").get(0).paused && $("#insert_audio").get(0).play(),
...
//                    c.src = "./Sword_small_" + this.level + ".png?version=2",
            ...
            tapHandle: function(evt) {
                var that = this;
            	if(nowMid && nowMid>0 && isAddRecored==0){
                	$.ajax({
                		type:'post',
                		url:initUrl,
                		...
                		success:function(data){
                			...
                			if(data.code==1){
                				...
                				$("#gameSuccessMsgText").html(data.msg);
                ...
                if(this.isCanTap){
                   this.isCanTap = false;
                    ...
                    var num = parseInt(params_json.levelArray[1]-n);
                        ...
                        console.log(initUrl);
                        console.log(num);
                        console.log(_.lastJudgeNum);
                        console.log(this.levelA);
                        console.log('t:::end:::::');

                    if(num<=_.lastJudgeNum){
                        if(this.nowSpeed>0){
                            this.nowSpeed = 0.1
                        }else{
                            this.nowSpeed = -0.1
                        ...
                        if(num == _.lastJudgeNum && this.levelA==2){
                                console.log(':::post data:::::');
                                console.log(this.levelA+1);
                                console.log(initUrl + '&acttttt='+(that.levelA+1));
                                console.log(':::post data end:::::');
                        	$.ajax({
	                    		type:'post',
	                    		url:initUrl + '&acttttt='+(that.levelA+1),
	                    		data:{op: 'ing', logId: logId, mid: nowMid, oid: nowOid,level:that.levelA+1},
	                    		success:function(data){
                                ...
                                console.log(nowMid);
                                console.log(nowOid);
                                console.log(that.levelA+1);
                                console.log(':::data end:::::');

	                    			if(data == 0){
                                        that.gameOverHandle && that.gameOverHandle.call(that, that.level-1);	
	                    				$.ajax({type:'post',url:initUrl,data:{op: 'fail', logId: logId, mid: nowMid, oid: nowOid, level:this.levelA+1}})
	                    			}else{sparams=data};
	                    		}
	                		})
                        }
                    ...
                    this.AimBullet.newY = d.y + o;if(sparams==2&&num==1){this.rotationSpeed=(this.rotationSpeed>0?0.1:-0.1),sparams++};
                }
            },
